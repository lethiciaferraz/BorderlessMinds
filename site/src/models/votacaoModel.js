...
function buscarUltimasMedidas(idvotacao ) {
    ...
    if (process.env.AMBIENTE_PROCESSO == "producao") {
        ...
        instrucaoSql = `select idioma, count(fkidioma) as 'Quantidade_Idiomas' from votacao join idioma on fkidioma=ididioma group by idioma;
    `;
    } else {
...
function cadastrar(idioma, idusuario) {
    ...
    //  e na ordem de inserção dos dados.
    var instrucao = `
        INSERT INTO votacao (fkidioma, fkusuario) VALUES (${idioma},${idusuario});
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
